...
async function updateShopSettings(updates) {
  await database.mutate((db) => {
    ...
    if (updates.supportWhatsapp !== undefined) db.settings.supportWhatsapp = updates.supportWhatsapp;
    if (updates.supportTelegram !== undefined) db.settings.supportTelegram = updates.supportTelegram;
  });
